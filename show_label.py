...
for i in range(10):
    img_input_np, part_roi_input, labellist,img_path = data_lei.next_batch(1)
    ...
    for j in range(1):#(part_roi_input.shape[1]):
        ...
        cv2.rectangle(img_ori, (int(x1), int()), (int(x1+w), int(y1+w)), (0, 255, 0), 2)
    #print()
    cv2.imshow('fsa',img_ori)
    cv2.waitKey(0)
    pass